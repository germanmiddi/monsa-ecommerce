<template>
	<div class="bg-white font-exo">
						...
						<div class="border-t border-gray-200 py-6 px-4 space-y-6">
							...
							<div class="flow-root">
								<a :href="route('aboutus')"
									class="-m-2 p-2 block font-medium text-gray-900">Nosotros</a>
							</div>
							<div class="flow-root">
								<a :href="route('blog.list')"
									class="-m-2 p-2 block font-medium text-gray-900">Novedades</a>
							</div>
							<div class="flow-root">
								<a :href="route('contacto')"
									class="-m-2 p-2 block font-medium text-gray-900">Contacto</a>
							</div>
						</div>
					...
					<div class="relative z-10 mx-auto h-14  px-4 py-4 flex items-center justify-end sm:px-6 lg:px-8">
						...
						<div v-else class="flex items-center space-x-6">
							<a :href="route('dashboard')"
								class="text-sm font-medium text-white hover:text-gray-100">Administrar</a>
							<div @click="logout"
								class="text-sm font-medium text-white hover:text-gray-100 cursor-pointer">Salir</div>
						</div>
					</div>
				</div>
							...
							<div class="hidden h-full lg:flex">
								...
								<PopoverGroup class="px-4 bottom-0 inset-x-0">
									<div class="h-full flex justify-center space-x-8">
										<a v-if="$page.props.show_module_store" :href="route('store')"
											class="flex items-center text-sm font-bold text-gray-50 hover:text-monsa-blue uppercase">Tienda</a>
										<a v-if="!$page.props.show_module_store" :href="route('home')"
							...
							<div class="flex-1 flex items-center justify-end">
								<div class="flex items-center lg:ml-8">
									...
									<div class="ml-4 flow-root lg:ml-8">
										<a v-if="$page.props.show_module_store" :href="route('checkout')"
											class="group -m-2 p-2 flex items-center hover:text-monsa-yellow">
											<ShoppingBagIcon
												class="flex-shrink-0 h-6 w-6 text-gray-50 group-hover:text-gray-100"
												aria-hidden="true" />
											<span
												class="ml-2 text-sm font-medium text-gray-50 group-hover:text-gray-100">{{
													totalItems }}</span>
										</a>
									</div>
								</div>
					...
					<div
							...
							<div class="grid grid-cols-1 md:grid-cols-3 mt-10 ">
								<div class="col-span-3 md:col-span-1">
									...
									<ul class="mt-6 space-y-2">
										...
										<li><a :href="route('aboutus')" class="text-sm hover:underline">Nosotros</a>
										</li>
										<li><a :href="route('blog.list')" class="text-sm hover:underline">Novedades</a>
										</li>
								...
								<div class="col-span-3 md:col-span-2">
									...
									<ul class="mt-6 space-y-2">
										<li><a :href="route('page', 'terminos-y-condiciones')"
												class="text-sm hover:underline">Términos y Condiciones</a></li>
										<li><a :href="route('page', 'politica-de-privacidad')"
												class="text-sm hover:underline">Política de Privacidad</a></li>
										<li><a :href="route('page', 'politica-de-devoluciones-y-reembolsos')"
												class="text-sm hover:underline">Política de Devoluciones y
												Reembolsos</a></li>
										<li><a :href="route('page', 'politica-de-envios')"
												class="text-sm hover:underline">Política de Envíos</a></li>
										<li><a :href="route('page', 'preguntas-frecuentes')"
												class="text-sm hover:underline">Preguntas Frecuentes</a></li>
									</ul>
								</div>
							</div>
						</div>
						<div class="col-span-1 md:col-span-4">
							<div class="flex items-center justify-end  space-x-3">
								<a :href="this.instagram" target="_blank"><svg class="w-12 h-12" viewBox="0 0 20 20"
								...
								<a :href="this.facebook" target="_blank">
									<svg class="w-12 h-12" viewBox="0 0 20 20" fill="none"
							...
							<div class="mt-8">
								...
								<p class="mt-6 text-sm text-white">Recibí las últimas novedades y promociones que
									tenemos para vos.</p>
								<div class="flex" v-if="!alertMessage">
									<input id="email-address" type="text" autocomplete="email" v-model="email"
										class="appearance-none border-white bg-monsa-blue min-w-0 w-full border border-bottom-1 rounded-md shadow-sm py-2 px-4 text-base text-white " />
									<div class="ml-4 flex-shrink-0">
										<button @click="subscribe"
											class="w-full bg-white border border-transparent rounded-md shadow-sm py-2 px-4 flex items-center justify-center text-base font-medium text-monsa-blue hover:bg-monsa-blue hover:text-white hover:border-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Suscribirse</button>
									</div>
								</div>
								<Alert :message="alertMessage" :type="alertType" @clear="clearMessage" />
							</div>
							<div class="mt-8 w-16">
								<a href="http://qr.afip.gob.ar/?qr=TCwMdwjOQ0Ro_jSB62EtYw,," target="_F960AFIPInfo"><img
										src="http://www.afip.gob.ar/images/f960/DATAWEB.jpg" border="0"></a>
							</div>
						</div>
					</div>

					<div class="mt-6 text-sm">Monsa S.R.L. - Av. Castañares 6140 - 1439 - Ciudad Autónoma de Buenos
						Aires, Argentina</div>
				</div>
		...
		<whatsappbtn :link="this.whatsapp" />
	</div>
</template>
