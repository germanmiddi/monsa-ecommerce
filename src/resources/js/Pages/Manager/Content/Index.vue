  ...
  <main class="max-w-7xl mx-auto pb-10 lg:py-12 lg:px-8">
    <div class="lg:grid lg:grid-cols-12 lg:gap-x-5">
        ...
        <nav class="space-y-1">
          <a v-for="(item, index) in asideItems" :key="item.name" @click.prevent="selectItem(index)"
            ...
            <component :is="item.icon"
              :class="[selectedIndex === index ? 'text-indigo-500' : 'text-gray-400 group-hover:text-gray-500', 'flex-shrink-0 -ml-1 mr-3 h-6 w-6']"
              aria-hidden="true" 
              />
      ...
      <div class="space-y-6 sm:px-6 lg:px-0 lg:col-span-9">
        <component @message="messageToast" 
...
import PagesCreate from '../Pages/Create.vue'
import PagesEdit from '../Pages/Edit.vue'  // Import PagesEdit
...
const staticAsideItems = [
  ...
  { name: 'Crear Página', icon: CubeIcon, componentName: 'PagesCreate' },
]
...
export default {
  ...
  methods: {
    async getPages() {
      ...
      this.updateAsideItems();

    },

    updateAsideItems() {
      this.asideItems = [
        ...this.staticAsideItems,
        ...this.pages.map(page => ({
          name: page.title,
          icon: CubeIcon,
          componentName: 'PagesEdit',
          pageId: page.id  // Add pageId to each page item
        }))
      ];
    },
